...
for i in range(1, len(idx0) + 1):
    if i == len(idx0):
        for j in range(N):
            ...
            V1_error_selfsupervised[i - 1][j] = np.abs(U_real[0, idx0[i - 1]:][j] - U_pred_selfsupervised[0, idx0[i - 1]:][j])
            V2_error_selfsupervised[i - 1][j] = np.abs(U_real[1, idx0[i - 1]:][j] - U_pred_selfsupervised[1, idx0[i - 1]:][j])
