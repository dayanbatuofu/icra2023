...
import training_selfsupervised, training_supervised, training_hybrid, training_valuehardening
...
if Selfsupervised == True:
    ...
    selfsupervised_dataset = dataio.IntersectionHJI_SelfSupervised(numpoints=61000,
    ...
    loss_fn_selfsupervised = loss_functions.initialize_intersection_HJI_selfsupervised(selfsupervised_dataset, Weight)

    path = 'experiment_HJI_' + opt.model + '_selfsupervised_HD' + '/'
...
if Supervised == True:
    ...
    path = 'experiment_HJI_' + opt.model + '_supervised_HD' + '/'
    root_path = os.path.join(opt.logging_root, path)
...
if Hybrid == True:
    ...
    hybrid_dataset = dataio.IntersectionHJI_Hybrid(numpoints=30000, tMin=opt.tMin, tMax=opt.tMax,
    ...
    path = 'experiment_HJI_' + opt.model + '_hybrid_HD' + '/'
    root_path = os.path.join(opt.logging_root, path)
...
if Valuehardening == True:
    ...
    valuehardening_dataset = dataio.IntersectionHJI_SelfSupervised(numpoints=61000,
                                     pretrain=opt.pretrain, tMin=opt.tMin,
                                     tMax=opt.tMax, counter_start=opt.counter_start,
                                     counter_end=opt.counter_end,
                                     pretrain_iters=opt.pretrain_iters, seed=opt.seed,
    ...
    for i in range(len(gamma)):
        if i == 0:
            ...
            path = 'experiment_HJI_' + opt.model + '_valuehardening_HD' + \
